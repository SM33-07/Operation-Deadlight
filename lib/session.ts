...
export async function getSession(): Promise<SessionData | null> {
  ...
  const dbSessionId = rawSessionId ? verifyCookie(rawSessionId) : null
  ...
  if (!isDbAvailable) {
    ...
    if (demoStateRaw) {
      try {
        ...
        return { ...parsed, userId: dbSessionId }
      } catch {
        return { ...DEFAULT_DEMO_STATE, userId: dbSessionId }
      }
    }
    return { ...DEFAULT_DEMO_STATE, userId: dbSessionId }
  }
  ...
  try {
    const userRows = await db.select().from(users).where(eq(users.id, dbSessionId))
    let user = userRows[0]
    
    if (!user) {
      if (true) { // Auto-create default user row if missing
        ...
        const lbRows = await db.select().from(leaderboard).where(eq(leaderboard.userId, dbSessionId))
        if (lbRows.length === 0) {
        ...
        const progressList = await db.select().from(timelineProgress).where(eq(timelineProgress.userId, dbSessionId))
    ...
    const recoveredRows = await db.select().from(fragments).where(eq(fragments.userId, dbSessionId))
    const recovered = recoveredRows.map((r: any) => r.timelineId)

    // Get wrong attempts to calculate integrity
    const events = await db.select().from(puzzleEvents).where(eq(puzzleEvents.userId, dbSessionId))
    ...
    const lbEntryRows = await db.select().from(leaderboard).where(eq(leaderboard.userId, dbSessionId))
